...
SetTimer:
LC4AA:  STA Timer2              ;Set Timer2. Frames to wait is value stored in A*10.
LC4AC:  STX NextRoutine         ;Save routine to jump to after Timer2 expires.
...
LC4E0:  BEQ MMCWriteRegExit     ;Exit if zero(no bank switch issued). else Y contains bank#+1.
...
LC597:  LDY #$0D                ;
LC599:* LDA MetroidData,y       ;Load info from table below into ram
LC59C:  STA MetroidDataRam,y 
LC59F:  DEY                     ;
LC5A0:  BPL -                   ;
        lda #$09
        sta SpareMemD1
...
.scope
    HandleAreaRoutine:
        lda #TourianBank
        cmp CurrentBank
        bne _skip          ;Saves 7 cycles in non-tourian banks
            jsr AreaRoutine                 ;Only Tourian actually has one of these
    _skip:
.scend
...
; Would gain nothing by inlining this
jsr CheckMissileToggle

; Could be inlined, but would only save like 10 cycles
...
CheckMissileToggle:
    lda MissileCount
    beq SelectSamusPalExit       ; exit if Samus has no missiles
    lda Joy1Change
    ora Joy1Retrig
    and #$20    
    beq SelectSamusPalExit       ; exit if SELECT not pressed
    lda MissileToggle
...
SamusStandTable_HiBytes:
    .byte >SetSamusExplodeExit, >SetSamusRun, >SetSamusJump, >SetSamusRoll, >SetSamusPntUp
...
LD8A3:
    ...
    lda #$08
    sec
    .byte $FF, $00, $03     ; Using illigal opcode here -> ISC ObjAction,x
    bne +
    lda #$23
    sta $0303,x
    lda #an_SamusFront
...
LE0C3:  
    ...
    DisplayBarLoop:                         ; Octo loop to save cycles
        lda DataDisplayTbl_Reversed-1, y    ; not a full unroll because bytes are precious
        sta SpriteRAM,x

        lda DataDisplayTbl_Reversed-2, y
        sta SpriteRAM+1,x

        lda DataDisplayTbl_Reversed-3, y
        sta SpriteRAM+2,x

        lda DataDisplayTbl_Reversed-4, y
        sta SpriteRAM+3,x

        lda DataDisplayTbl_Reversed-5, y
        sta SpriteRAM+4,x

        lda DataDisplayTbl_Reversed-6, y
        sta SpriteRAM+5,x

        lda DataDisplayTbl_Reversed-7, y
        sta SpriteRAM+6,x

        lda DataDisplayTbl_Reversed-8, y
        sta SpriteRAM+7,x

        lda IdentityTable+8, x
        tax

        lda IdentityTable-8, y
        tay
...
*   ldx ScrollY
    ...
    ldx #240    ; new Y coord
*   dex
    cpx #MidScreenVertical + 12      ; Load just a bit before you reach the half way point vertically
...
; If we don't need to write too many bytes
; we can avoid the heavy "jsr writePPUByte" function
; and take a fast path
lda #PPUMaxSize - 4
sbc PPUStrIndex
cmp $05
bcs Inc_1_Fast_Loop_Setup
...
*   dec $05
    bne Inc_1_Loop
    beq After_Inc_Loop      ;always branch

Inc_1_Fast_Loop_Setup:
    clc
Inc_1_Fast_Loop:
    lda ($00),y
    sta PPUDataString, x
...
_loadNameAddrs3:
    ...
    ldx #$67                 ;High byte of RoomRAMB(cart RAM)
    bne _afterload           ;Always branch
_loadNameAddrs0:
    lda #$23                 ;High byte of nametable #0(PPU).
    ldx #$63                 ;High byte of RoomRAMA(cart RAM)
_afterload:
    sta $03             ;#$23 for attrib table 0, #$2F for attrib table 3.
    stx $01             ;#$63 for RoomRAMA, #$67 for RoomRAMB(Attrib tables).

.scend

LE5FC:  ldy #$01            ;
LE5FE:  sty PPUDataPending      ;Data pending = YES.
...
LE60D:  lda #$20                ;Length of data to write(1 row of attrib data).
        sta PPUDataString, x
        inx
        dey                    ;y = 0 here; Reset index into data string.
...
; ==== OLD ====
;LEA2D:  cmp #$FE                    ; Probably don't need this #$FE check
;        bcs RoomFinishedExit
...
LEA45:  sta _originalRoomPtr_Lo     ;Base copied from $959A to $3B.
...
LEAD0:  sta $01             ;
        ...
        bne AfterDrawRoom
            inc $01
; End of Draw Room
        bne AfterDrawRoom   ; print_stuff_addr_1 in BuildRoomAnalyzer.lua  (Room end no enemies path)
...
;Choose Routine
    beq GetEnemyDataExit        ; Handle the 0 case
    TAX
...
EnemyLoopTable_HiBytes:
    .byte >LoadEnemy, >LoadDoor, >ExitSub, >LoadElevator, >ExitSub, >LoadStatues, >ZebHole 
...
LEB09:  bne EnemyLoop           ;branch always
...
DoOneEnemy:
    stx PageIndex
    cpy #$03
    bcs ChooseEnemySubroutine_SkipPageIndexLoad
; inlined what used to be jsr LF37F
    lda EnAttr_05,x
    and #$02
    bne ChooseEnemySubroutine_SkipPageIndexLoad
...
DoOneEnemyTableHiByte:
    .byte >EnemyRoutine_1, >EnemyRoutine_2, >EnemyRoutine_3, >EnemyRoutine_4, >EnemyRoutine_5, >EnemyRoutine_6 
...
.scope
    ...
    sta PageIndex
    ldx #$0C
SpinnerDestructionLoop:
    jsr DoOneSpinnerDestruction
    dex
    dex
    dex
    dex
    bne SpinnerDestructionLoop
...
DoOneTile:
    ...
    ; 0                     = RTS
    ; 1 (001)               = LFE3D
    ; 2 and 4 (010 and 100) = LFE54
    ; 3 (011)               = LFE59
    ; 5 (101)               = LFE83

    beq Exit28
    bit IdentityTable+1 ; works like "Bit #$01" would work
    beq LFE54
    cmp #$03
    beq LFE59
    bcs LFE83
...
LFE54:
    ...
    * STA TileAnimDelay,x
    LDY TileAnimIndex,x
    LDA TileBlastAnim,y
    ; Don't need check for FE, can just check byte 7 with BMI
    BMI ++
    STA TileAnimFrame,x
    INY
    TYA
...
FFLabel:          ; Sometimes you just need to point to $FF.
    .byte $FF
